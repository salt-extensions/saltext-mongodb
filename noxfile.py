...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/mongodb/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/mongodb/*")
            ...
            #    "--include=src/saltext/mongodb/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/mongodb/config/schemas",
    )
